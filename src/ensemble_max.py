...
def get_answer(model1_pred, model2_pred):
    ...
    for question in questions:
        ...
        for start in common_start_keys:
            highest_score = max(model1_pred[question]["start"][start], model2_pred[question]["start"][start])
            common_start[start] = highest_score
        for end in common_end_keys:
            highest_score = max(model1_pred[question]["end"][end], model2_pred[question]["end"][end])
            common_end[end] = highest_score
...
if __name__ == "__main__":
    ...
    with open("./ensemble_max_output_roberta_xlnet.json", 'w') as f:
